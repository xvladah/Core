...
class TConsts
{
	...
	{
		return '- '.__('box.all', 'Vše').' -';
	}

	public static function __MY() :string
	{
		return '- '.__('box.my', 'Moje').' -';
	}
	
	public static function CodeStatuses() :array
	{
		return [
                TUSer::CODE_STATUS_INACTIVE 	     => ['name'=>__('box.verification_code.inactive', 'Neaktivní')],
				TUSer::CODE_STATUS_ACTIVE_EMAIL 	 => ['name'=>__('box.verification_code.active.email', 'E-mail')],
                TUSer::CODE_STATUS_PROXY_EMAIL       => ['name'=>__('box.verification_code.proxy', 'E-mail (pouze proxy)')],
                TUSer::CODE_STATUS_ACTIVE_AUTHENTICATOR  => ['name'=>__('box.verification_code.active.microsoft', 'Authenticator')],
                TUSer::CODE_STATUS_PROXY_AUTHENTICATOR   => ['name'=>__('box.verification_code.proxy.microsoft', 'Authenticator (pouze proxy)')],
		];
	}
	...
	{
		return [
			self::NOTICE_TYPE_INFO 		=> ['name'=>__('notice.type.info', 'Informace'),	'class'=>'ico-info'],
			self::NOTICE_TYPE_WARNING	=> ['name'=>__('notice.type.warning', 'Varování'),	'class'=>'ico-wrn'],
			self::NOTICE_TYPE_ERROR		=> ['name'=>__('notice.type.error', 'Chyba'),		'class'=>'ico-err']
		];
	}
	...
	{
		return [
			self::NOTICE_STATUS_NEW		=> ['name'=>__('notice.status.new', 'Nové'),			'class'=>'status-silver'],
			self::NOTICE_STATUS_READ	=> ['name'=>__('notice.status.read', 'Přečteno'),		'class'=>'status-blue'],
			self::NOTICE_STATUS_WORK	=> ['name'=>__('notice.status.work', 'Rozpracováno'),	'class'=>'status-orange'],
			self::NOTICE_STATUS_READY	=> ['name'=>__('notice.status.ready', 'Vyřízeno'),		'class'=>'status-green'],
			self::NOTICE_STATUS_DENIED	=> ['name'=>__('notice.status.denied', 'Zamítnuto'),	'class'=>'status-red'],
	...
	{
		return [
			self::LANG_DEFAULT	=> ['name'=>__('lang.default', 'Defaultní (Čeština)'),	'file'=>null,	'class'=>''],
	...
	{
		return [
			self::STATUS_ACTIVE 	=> ['name' => __('box.active', 'Aktivní')],
			self::STATUS_INACTIVE	=> ['name' => __('box.inactive', 'Neaktivní')]
		];
	}
	...
	{
		$result[self::STATUS_ALL]			= '- '.__('box.all', 'Vše').' -';
		$result[':'.self::STATUS_ACTIVE]	= __('box.active_only', 'Pouze aktivní');
		$result[':'.self::STATUS_INACTIVE]	= __('box.inactive_only', 'Pouze neaktivní');
	...
	public static function getStatusIco(?int $status_id) :string
	{
		switch($status_id)
		{
			...
			case '-1'	: $class = 'status-red';	$title = __('status.error', 'Chyba'); break;
